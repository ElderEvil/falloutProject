...
- ❌ Datetime deprecation fix (deferred)
- ❌ Incident service tests (deferred)
- ❌ Game loop tests (deferred)
...
## Task 1: Database Initialization Tests 💾

**Priority:** HIGH | **Effort:** 2-3 hours | **Coverage:** 0% → 75%
...
class TestInitDB:
    ...
    async def test_complete_hierarchy(self, async_session: AsyncSession):
        """Verify complete entity hierarchy: user → vault → rooms → dwellers → equipment."""
        await init_db(async_session)
...
- [ ] Target: ≥75% coverage for init_db.py

---

## Task 2: Unique Room Verification Tests 🏠

**Priority:** MEDIUM | **Effort:** 1-2 hours | **Coverage:** Improve from ~70% → 95%
...
- **Specific Modules:**
  ...
  - `app/api/v1/endpoints/room.py`: ~70% → 95%

### Quality
- ✅ Database seeding fully verified
- ✅ Unique room logic comprehensively tested
- ✅ All tests passing
