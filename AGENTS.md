...
1. Never push to git without explicit approval, and never merge a pull request without explicit approval for that specific PR. Approval of the underlying work (e.g. "do it", "fix X") authorizes the change, not the merge: open the PR, get CI green, and hand it over for review — the maintainer merges.
2. After backend API changes: `cd frontend && pnpm run types:generate`.
3. Small, test-backed changes; follow existing patterns; commit messages `feat:`/`fix:`/`chore:`; branch prefixes `feat/`/`fix/`/`chore/`. When touching Python, clear that file's `ty` errors/warnings as incremental cleanup (verify: `uv run ty check --output-format concise 2>&1 | grep <path>` empty) — except `datetime.utcnow` deprecation (naive-UTC convention, see README); never widen to unrelated files. The `ty` prek hook enforces this on changed `backend/**/*.py` files except `app/tests/`, `app/alembic/`, and `app/crud/` (ty cannot see SQLModel/SQLAlchemy instrumentation — untouched `crud/dweller.py` alone reports ~90 errors, so CRUD stays exempt until a dedicated descriptor-baseline batch): error-level diagnostics block the commit, warnings stay advisory so the exempt `datetime.utcnow` deprecation cannot block unrelated work. Endpoints annotate the Python type actually returned (ORM model / `Sequence`); the `response_model=` decorator owns the wire shape — never add `model_validate` conversions just to mirror it.
...
5. **DRY-first, reduce-LOC discipline (v2.35+):** before writing code, search for an existing implementation of the same behaviour and reuse or extend it instead of reinventing it. Prefer reducing source LOC when it is possible without hurting readability; don't count generated files, lockfiles, or format-only changes. Prefer fewer files too: do not split into more modules/files unless readability genuinely suffers — but never below the architecture floor from rule 4.
