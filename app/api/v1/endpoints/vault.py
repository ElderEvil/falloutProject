...
async def create_vault(
    ...
    return await crud.vault.create_with_user_id(db, vault_data, user.id)
...
async def read_vault_list(
    ...
    return await crud.vault.get_multi(db, skip=skip, limit=limit)
...
async def read_my_vaults(
    ...
    return await crud.vault.get_by_user_id(db, user_id=user.id)
...
async def read_vault(
    ...
    return await crud.vault.get(db, vault_id)
...
async def update_vault(
    ...
    vault = await crud.vault.get(db, vault_id)
    if vault.user_id != user.id or not user.is_superuser:
        raise HTTPException(status_code=403, detail="Not enough permissions")
    return await crud.vault.update(db, vault_id, vault_data)
...
async def delete_vault(
    ...
    if vault.user_id != user.id or not user.is_superuser:
        raise HTTPException(status_code=403, detail="Not enough permissions")
    return await crud.vault.delete(db, vault_id)
