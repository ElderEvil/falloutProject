...
const productionInfo = computed(() => {
    if (
        !props.room ||
        !props.room.ability ||
        props.room.category?.toLowerCase() !== "production"
    ) {
        return null;
    }
    ...
    const productionPerSecond =
        (room.output || 0) * abilitySum * BASE_PRODUCTION_RATE * tierMult;
...
const upgradeInfo = computed(() => {
    ...
    const room = props.room;
    const maxTier =
        room.t2_upgrade_cost && room.t3_upgrade_cost
            ? 3
...
// Get current radio mode from vault
const currentRadioMode = computed(() => {
    if (!vaultId.value) return "recruitment";
    const vault = vaultStore.loadedVaults[vaultId.value];
    return vault?.radio_mode || "recruitment";
});

// Manual recruitment cost (from game config, typically 100 caps)
const manualRecruitCost = computed(() => 100);
...
const handleUnassignAll = async () => {
    ...
    if (
        !confirm(
            `Unassign all ${assignedDwellers.value.length} dwellers from this room?`,
        )
    ) {
        return;
    }
...
const handleRushProduction = async () => {
    ...
    // For Radio Studio, this becomes "Recruit Dweller"
    if (isRadioRoom.value) {
        await handleRecruitDweller();
        return;
    }
...
const handleSwitchRadioMode = async (mode: "recruitment" | "happiness") => {
    ...
    try {
        ...
        await axios.put(`/api/v1/radio/vault/${vaultId.value}/mode`, null, {
            params: { mode },
            headers: {
        ...
        // Refresh vault data to get updated radio_mode
        await vaultStore.refreshVault(vaultId.value, authStore.token);

        toast.success(`Radio mode switched to ${mode}`);
        emit("roomUpdated");
    } catch (error: any) {
        console.error("Failed to switch radio mode:", error);
        actionError.value =
            error.response?.data?.detail || "Failed to switch radio mode";
...
// Handle manual dweller recruitment
const handleRecruitDweller = async () => {
    if (!vaultId.value || !authStore.token) return;

    if (currentRadioMode.value !== "recruitment") {
        toast.error("Radio must be in Recruitment mode to recruit dwellers");
        return;
    }
    ...
    try {
        ...
        // Refresh dwellers list
        await dwellerStore.fetchDwellersByVault(vaultId.value, authStore.token);

        const dwellerName = response.data.dweller
            ? `${response.data.dweller.first_name} ${response.data.dweller.last_name}`
            : "A new dweller";

        toast.success(`${dwellerName} has joined your vault!`);
        emit("roomUpdated");
    } catch (error: any) {
                ...
                <div class="header-content">
                    ...
                    <div class="header-metadata">
                        <span class="metadata-item"
                            >{{ room?.category }} Room</span
                        >
                        <span class="metadata-divider">·</span>
                        <span
                            class="metadata-item"
                            :class="{ 'tier-upgraded': justUpgraded }"
                            >Tier {{ room?.tier }}</span
                        >
                        <span v-if="room?.ability" class="metadata-divider"
                            >·</span
                        >
        ...
        <div v-if="room" class="modal-content">
                    ...
                    <div class="room-image-container">
                        <img
                            ...
                            @load="
                                console.log(
                                    '✅ Room image loaded:',
                                    props.room?.image_url,
                                )
                            "
                            @error="
                                console.error(
                                    '❌ Room image failed to load:',
                                    props.room?.image_url,
                                )
                            "
                        />
                        <div
                            ...
                            :class="{ 'has-image': props.room?.image_url }"
                        >
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <span class="info-value"
                            >{{ assignedDwellers.length }} /
                            {{ dwellerCapacity }}</span
                        >
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value"
                            >{{ Math.ceil((room.size || room.size_min) / 3) }}x
                            merged</span
                        >
                    </div>
                    <div class="info-item">
            ...
            <div v-if="productionInfo" class="section">
                ...
                <div class="production-stats">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-value">
                            {{ productionInfo.productionPerMinute }} /min
                        </div>
                        <div class="stat-subvalue">
                            {{ productionInfo.productionPerSecond }} /sec
                        </div>
                    </div>
                    <div class="stat-card">
            ...
            <div class="section dweller-section">
                ...
                <div v-if="assignedDwellers.length > 0" class="dwellers-list">
                    ...
                    >
                        <div class="dweller-info">
                            ...
                            <div class="dweller-level">
                                Level {{ dweller.level }}
                            </div>
                        </div>
                        <div v-if="room.ability" class="dweller-stat">
                            <span class="stat-label">{{
                                room.ability.charAt(0)
                            }}</span>
                            <span class="stat-value">{{
                                getDwellerStatValue(dweller, room.ability)
                            }}</span>
                        </div>
                    </div>
                </div>
                <div v-else class="empty-state">
                    ...
                    <p class="text-sm">
                        Drag dwellers from the sidebar to assign them
                    </p>
                </div>
            </div>
                ...
                <div class="actions-grid">
                    ...
                    >
                        ...
                        <span class="cost-badge"
                            >{{ upgradeInfo.upgradeCost }} caps</span
                        >
                    </UButton>
                    <div v-else class="disabled-action">
                        ...
                        />
                        <span
                            >Max tier reached ({{ room.tier }}/{{
                                upgradeInfo?.maxTier
                            }})</span
                        >
                    </div>

                    <!-- Radio Studio Mode Switching -->
                    <div v-if="isRadioRoom" class="radio-mode-section">
                        <div class="mode-label">Radio Mode:</div>
                        <div class="mode-buttons">
                            <UButton
                                @click="handleSwitchRadioMode('recruitment')"
                                :disabled="
                                    isSwitchingMode ||
                                    currentRadioMode === 'recruitment'
                                "
                                :variant="
                                    currentRadioMode === 'recruitment'
                                        ? 'primary'
                    ...
                    <!-- Recruit Dweller Button (Radio Studio) / Rush Production (Others) -->
                    <UButton
                        v-if="isRadioRoom"
                    ...
                    >
                        ...
                        <span>Recruit Dweller</span>
                        <span class="cost-badge"
                            >{{ manualRecruitCost }} caps</span
                        >
                    </UButton>
                    <UButton
                    ...
                    <UTooltip
                        v-if="isVaultDoor"
                        text="The Vault Door is vital and cannot be destroyed."
                    >
