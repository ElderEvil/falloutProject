...
**Deliverables:**
- 24 new tests added (570 → 594)
- 15-20% coverage increase (44.5% → 60-65%)
- Zero Python 3.13+ deprecation warnings
- Clean foundation for Tier 2/3 work

---

## Task 1: Datetime Deprecation Fix ⏰
...
2. Replace pattern: `datetime.utcnow()` → `datetime.now(timezone.utc)`
3. For Field defaults: `default_factory=datetime.utcnow` → `default_factory=lambda: datetime.now(timezone.utc)`
...
## Task 2: Database Initialization Tests 💾
...
async def test_init_db_creates_dwellers_for_rooms(async_session: AsyncSession):
    ...
    assert len(dwellers) == 6  # 3 rooms × 2 dwellers
...
## Task 3: Unique Room Verification Tests 🏠
...
- ✅ Database seeding fully tested
- ✅ Unique room logic verified
- ✅ All tests passing
- ✅ Ruff linting clean
