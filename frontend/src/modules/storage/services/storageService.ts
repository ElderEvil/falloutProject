...
export const storageService = {
    ...
    async getStorageSpace(vaultId: string): Promise<StorageSpaceResponse> {
        const response = await axios.get<StorageSpaceResponse>(
            `/api/v1/storage/vault/${vaultId}/space`,
        );
    ...
    async getStorageItems(vaultId: string): Promise<StorageItemsResponse> {
        const response = await axios.get<StorageItemsResponse>(
            `/api/v1/storage/vault/${vaultId}/items`,
        );
        return response.data;
    },
