...
Revision ID: 8f5ca829bb79
Revises:
Create Date: 2024-06-22 08:24:07.710161
...
def upgrade() -> None:
    ...
    op.create_table('objective',
                    sa.Column('challenge', sa.String(length=32), nullable=False),
                    sa.Column('reward', sa.String(length=32), nullable=False),
    ...
    op.create_table('questobjective',
                    sa.Column('title', sa.String(length=64), nullable=False),
                    sa.Column('id', sa.UUID(as_uuid=True), nullable=False),
                    sa.Column('quest_id', sa.UUID(as_uuid=True), nullable=False),
    ...
    op.create_table('room',
                    ...
                    sa.Column('name', sa.String(length=32), nullable=False),
                    sa.Column('category',
                              sa.Enum('CAPACITY', 'CRAFTING', 'MISC', 'PRODUCTION', 'QUESTS', 'THEME', 'TRAINING',
                                      name='roomtypeenum'), nullable=False),
                    sa.Column('ability',
                              sa.Enum('STRENGTH', 'PERCEPTION', 'ENDURANCE', 'CHARISMA', 'INTELLIGENCE', 'AGILITY',
                                      'LUCK', name='specialenum'), nullable=True),
                    ...
                    sa.Column('id', sa.UUID(as_uuid=True), nullable=False),
                    sa.Column('vault_id', sa.UUID(as_uuid=True), nullable=False),
    ...
    op.create_table('vaultquestchaincompletionlink',
                    sa.Column('vault_id', sa.UUID(as_uuid=True), nullable=False),
                    sa.Column('quest_entity_id', sa.UUID(as_uuid=True), nullable=False),
                    sa.Column('is_completed', sa.Boolean(), nullable=False),
    ...
    op.create_table('vaultquestcompletionlink',
                    ...
                    sa.Column('is_completed', sa.Boolean(), nullable=False),
                    sa.Column('is_visible', sa.Boolean(), nullable=False),
                    sa.ForeignKeyConstraint(['quest_entity_id'], ['quest.id'], ),
                    sa.ForeignKeyConstraint(['vault_id'], ['vault.id'], ),
                    sa.PrimaryKeyConstraint('vault_id', 'quest_entity_id')
                    )
    ...
    op.create_table('junk',
                    ...
                    sa.Column('image_url', sqlmodel.sql.sqltypes.AutoString(length=255), nullable=True),
                    sa.Column('junk_type',
                              sa.Enum('CIRCUITRY', 'LEATHER', 'ADHESIVE', 'CLOTH', 'SCIENCE', 'STEEL', 'VALUABLES',
                                      name='junktypeenum'), nullable=False),
                    sa.Column('description', sqlmodel.sql.sqltypes.AutoString(length=255), nullable=False),
                    sa.Column('id', sa.UUID(as_uuid=True), nullable=False),
                    sa.Column('storage_id', sa.UUID(as_uuid=True), nullable=True),
    ...
    op.create_table('outfit',
                    ...
                    sa.Column('value', sa.Integer(), nullable=True),
                    sa.Column('image_url', sqlmodel.sql.sqltypes.AutoString(length=255), nullable=True),
                    sa.Column('outfit_type',
                              sa.Enum('COMMON', 'RARE', 'LEGENDARY', 'POWER_ARMOR', 'TIERED', name='outfittypeenum'),
                              nullable=True),
                    sa.Column('gender', sa.Enum('MALE', 'FEMALE', name='genderenum'), nullable=True),
                    sa.Column('id', sa.UUID(as_uuid=True), nullable=False),
                    sa.Column('dweller_id', sa.UUID(as_uuid=True), nullable=True),
                    sa.Column('storage_id', sa.UUID(as_uuid=True), nullable=True),
    ...
    op.create_table('weapon',
                    ...
                    sa.Column('value', sa.Integer(), nullable=True),
                    sa.Column('image_url', sqlmodel.sql.sqltypes.AutoString(length=255), nullable=True),
                    sa.Column('weapon_type', sa.Enum('MELEE', 'GUN', 'ENERGY', 'HEAVY', name='weapontypeenum'),
                              nullable=False),
                    sa.Column('weapon_subtype',
                              sa.Enum('BLUNT', 'EDGED', 'POINTED', 'PISTOL', 'RIFLE', 'SHOTGUN', 'AUTOMATIC',
                                      'EXPLOSIVE', 'FLAMER', name='weaponsubtypeenum'), nullable=False),
