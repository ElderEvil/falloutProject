...
class LootOverflowService:
    ...
    def value_of(self, loot_item: dict) -> int:
        """Caps paid for selling one held entry, priced from the shipped catalogs.
        ...
        match loot_item.get("item_type", "junk"):
            case "weapon":
                ...
                return (data.value or 0) * quantity
            case "outfit":
            ...
            case _:
                ...
                return game_config.exploration.get_junk_value(rarity) * quantity
    ...
    async def require_space(db_session: AsyncSession, vault_id: UUID4, quantity: int) -> Storage:
        ...
        storage = await crud_storage.get_storage_by_vault(db_session, vault_id)
