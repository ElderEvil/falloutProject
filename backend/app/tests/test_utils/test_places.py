...
class TestSchematicCoords:
    ...
    def test_band_and_one_decimal_resolution(self):
        for i in range(50):
    ...
    def test_distinct_names_give_distinct_coords(self):
...
class TestCollisionNudge:
    ...
    def test_free_base_returned_unchanged(self):
        base = (50.0, 50.0)
        assert collision_nudge(base, set()) == base
        assert collision_nudge(base, {(10.0, 10.0)}) == base

    def test_occupied_base_moves_off(self):
        base = (50.0, 50.0)
        occupied = {base}
...
class TestSeededVaultSpecs:
    ...
    def test_numbers_distinct_and_exclude_home(self):
        for i in range(10):
            home = i + 1
    ...
    def test_coords_avoid_home_origin_and_each_other(self):
        specs = seeded_vault_specs(self.VAULT_1, home_number=1)
