...
const rooms = computed(() =>
    Array.isArray(roomStore.rooms) ? roomStore.rooms : [],
);
...
const { selectedRoomId, toggleRoomSelection, destroyRoom } =
    useRoomInteractions();
const {
    hoverPosition,
    handleHover,
...
const placeRoom = async (x: number, y: number) => {
    ...
    try {
        ...
        assignmentError.value =
            error instanceof Error ? error.message : "Failed to build room";
        setTimeout(() => {
            assignmentError.value = null;
        }, 3000);
    }
...
const handleDrop = async (event: DragEvent, roomId: string) => {
    ...
    try {
        ...
        if (targetRoom) {
            ...
            const currentDwellers = dwellerStore.dwellers.filter(
                (d) => d.room_id === roomId,
            ).length;
        ...
        <div class="room-grid" :class="{ 'critical-power': isPowerOutage }">
            ...
            >
                <div class="room-content">
                    ...
                    <div class="room-info-overlay">
                        ...
                        <div v-if="room.tier" class="room-tier">
                            Tier {{ room.tier }}
                        </div>
                    </div>
                    <div
                        v-if="draggingOverRoomId === room.id"
                    ...
                    >
                        <Icon
                            :icon="
                                getIncidentIcon(getRoomIncident(room.id)!.type)
                            "
                            class="incident-icon"
                        />
            ...
            <div
                ...
                :class="{
                    'hover-preview': previewCells.some(
                        (previewCell) =>
                            previewCell.x === cell.x &&
                            previewCell.y === cell.y,
                    ),
                    'valid-placement':
                        isValidPlacement &&
                        previewCells.some(
            ...
            >
                ...
                <span class="locked-text"
                    >Locked Area - Future Expansion (Row
                    {{ GRID_ROWS + y - 1 }})</span
                >
            </div>
        </div>
...
.room-grid {
    ...
    grid-template-columns: repeat(
        8,
        minmax(140px, 1fr)
    ); /* 8 columns with larger min width */
